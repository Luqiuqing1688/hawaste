...
(function (win) {
	...
	*/
    var QD = jeDt.query = function () {
        function r(c, g) {
            g = g || document;
            if (!/^[\w\-_#]+$/.test(c) && g.querySelectorAll) return m(g.querySelectorAll(c));
            if (-1 < c.indexOf(",")) {
                for (var d = c.split(/,/g), a = [], b = 0, e = d.length; b < e; ++b) a = a.concat(r(d[b], g));
                return y(a)
            }
            var d = c.match(z), a = d.pop(), e = (a.match(t) || k)[1], f = !e && (a.match(u) || k)[1],
                b = !e && (a.match(v) || k)[1], a = c.match(/\[(?:[\w\-_][^=]+)=(?:[\'\[\]\w\-_]+)\]/g);
        ...
        var z = /(?:[\*\w\-\\.#]+)+(?:\[(?:[\w\-_][^=]+)=(?:[\'\[\]\w\-_]+)\])*|\*|>/gi,
            u = /^(?:[\w\-_]+)?\.([\w\-_]+)/, t = /^(?:[\w\-_]+)?#([\w\-_]+)/, v = /^([\w\*\-_]+)/,
            k = [null, null, null], x = /\[([\w\-_][^=]+)=([\'\[\]\w\-_]+)\]/, y = function () {
    ...
    InitDate.prototype = {
        ...
        setHtml: function (opts, self) {
            var that = this, weekHtml = "", date = new Date(), nowDateVal = jeDt.nowDate(null, opts.format),
                isformat = opts.format.match(/\w+|d+/g).join("-") == "YYYY-MM" ? true : false;
        ...
        onlyYMevents: function (that, opts, self, arrTime) {
            var ymPre = QD(Cell + " .jedateym .ymprev")[0], ymNext = QD(Cell + " .jedateym .ymnext")[0],
                ony = parseInt(arrTime[0]), onm = parseInt(arrTime[1]);
        ...
        getDateStr: function (y, m, d) {
            ...
            var pervMonthlastDay = getPervMonthLastDay(y, m),
                currentMonthDays = getPervMonthLastDay(y, parseInt(m) + 1);
            //上月最后几天循环
            var lastdays = pervMonthlastDay - pervLastDay;
            //判断是否超出允许的日期范围	
            var startDay = 1, minArr = parseArr(opts.minDate), maxArr = parseArr(opts.maxDate),
                endDay = currentMonthDays, thisDate = new Date(y, m, d), firstDate = new Date(y, m, 1),
                lastDate = new Date(y, m, currentMonthDays), minTime = new Date(minArr[0], minArr[1], minArr[2]),
                maxTime = new Date(maxArr[0], maxArr[1], maxArr[2]), minDateDay = minTime.getDate();
            if (minTime > lastDate) {
        ...
        events: function (that, opts, self, arrTime) {
            var yearPre = QD(Cell + " .yearprev")[0], yearNext = QD(Cell + " .yearnext")[0],
                monthPre = QD(Cell + " .monthprev")[0], monthNext = QD(Cell + " .monthnext")[0], newDate = new Date(),
                jedateyear = QD(Cell + " .jedateyear")[0], jedatemonth = QD(Cell + " .jedatemonth")[0],
                isformat = opts.format.match(/\w+|d+/g).join("-") == "YYYY-MM" ? true : false;
            if (!isformat) {
                //切换年
                ...
                jeDt.each(QD(Cell + " .jedatebot .jedatehms em"), function (i, cls) {
                    jeDt.on(cls, "click", function () {
                        var hmsStr = "", acton, hmscell = QD(Cell + " .jedateprophms")[0],
                            hmslen = jeDt.attr(cls, "data-hms"), hmsstxt = ["小时", "分钟", "秒数"],
            ...
            jeDt.on(QD(Cell + " .jedatebot .jedateok")[0], "click", function (ev) {
                ...
                if (!isformat) {
                    var okTimeArr = [],
                        okTime = [parseInt(jeDt.attr(jedateyear, "data-year")), parseInt(jeDt.attr(jedatemonth, "data-month")), arrTime[2]];
        ...
        YearAndMonth: function (that, opts, self, arrTime) {
            var jetopym = QD(Cell + " .jedatetopym")[0], jedateyy = QD(Cell + " .jedateyy")[0],
                jedatemm = QD(Cell + " .jedatemm")[0], jedateyear = QD(Cell + " .jedateyy .jedateyear")[0],
                jedatemonth = QD(Cell + " .jedatemm .jedatemonth")[0], mchri = QD(Cell + " .jedateymchri")[0],
                mchle = QD(Cell + " .jedateymchle")[0];
            ...
            jeDt.on(jedateyy, "click", function () {
                var YMlen = parseInt(jeDt.attr(jedateyy, "data-ym")),
                    yearAttr = parseInt(jeDt.attr(jedateyear, "data-year"));
                setYearMonth(yearAttr, YMlen);
                clickLiYears(jedateyear);
            });
            //下拉选择月
            jeDt.on(jedatemm, "click", function () {
                var YMlen = parseInt(jeDt.attr(jedatemm, "data-ym")),
                    yearAttr = parseInt(jeDt.attr(jedateyear, "data-year"));
                setYearMonth(yearAttr, YMlen);
                jeDt.each(QD(Cell + " .ymdropul li"), function (i, cls) {
                    jeDt.on(cls, "click", function (ev) {
        ...
        clickLiDays: function (that, opts, self) {
            jeDt.each(QD(Cell + " .jedaul li"), function (i, cls) {
                jeDt.on(cls, "click", function (ev) {
                    ...
                    var aty = parseInt(jeDt.attr(cls, "data-y")) | 0, atm = parseInt(jeDt.attr(cls, "data-m")) | 0,
                        atd = parseInt(jeDt.attr(cls, "data-d")) | 0;
